...
export function useScrollProgress(options: ScrollProgressOptions = {}) {
  ...
  const handleScroll = useCallback(() => {
    ...
    try {
      // Marquee 진행률 계산 - spaceSectionRef 사용
      if (marqueeRef.current && spaceSectionRef.current) {
        const spaceRect = safeGetBoundingRect(spaceSectionRef.current);
        if (spaceRect) {
  ...
  useEffect(() => {
    ...
    try {
      // 요소들 찾기 - 안전한 쿼리 셀렉터 사용 (한 번만 실행)
      spaceSectionRef.current = safeQuerySelector<HTMLElement>(container, spaceSectionSelector);
      ...
      if (!spaceSectionRef.current || !marqueeRef.current) {
  ...
  }, [handleScroll, handleResize, spaceSectionSelector, marqueeSectionSelector, videoReelSelector]);
