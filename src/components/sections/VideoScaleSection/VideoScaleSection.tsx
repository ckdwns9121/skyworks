...
interface VideoScaleSectionProps {
  videoSrc: string;
  text: string;
  index: number;
  dark?: boolean;
...
const VideoScaleSection = forwardRef<HTMLDivElement, VideoScaleSectionProps>(
  ({ videoSrc, text, index, dark = false }, ref) => {
    ...
    return (
      ...
      >
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center text-5xl font-bold pointer-events-none select-none"
          style={{ color: dark ? "#fff" : "#888", zIndex: 2 }}
        >
          {text}
        </div>
        <div
...
VideoScaleSection.displayName = "VideoScaleSection";

export default VideoScaleSection;
